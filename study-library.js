...
const settings = {
  ...
  guide: {title:'Complete guide',intro:'All 65 study-guide items, organized into seven topics. Try an answer before revealing it.',eyebrow:'YOUR COMPLETE STUDY REFERENCE',pdf:'complete-study-guide.pdf'}
};
const notes = {
  3:'Source check: this is the supplied guide’s “capacitive + lagging” pairing. Confirm this pairing with your instructor before memorizing it.',
  9:'Diagram-dependent: these line numbers refer to the source control-system diagram.',
  10:'Diagram-dependent: this line number refers to the source control-system diagram.',
  19:'Source check: the guide highlights OPEN LOOP, but its handwritten note questions “closed loop?” Confirm this answer with your instructor.'
};
let review = new Set();
try { const saved=JSON.parse(localStorage.getItem('test9-study-review-v1')); if(Array.isArray(saved))review=new Set(saved.filter(n=>Number.isInteger(n)&&n>=1&&n<=65)); } catch {}
export function renderLibrary(app,view) {
  const config=settings[view];
  const items=studyItems.filter(q=>view==='guide'||!q.missing);
  let topic='',search='',onlyReview=false;
  const revealed=new Set(view==='answers'?items.map(q=>q.number):[]);
  app.innerHTML=`<div class="library-heading"><p class="eyebrow">${config.eyebrow}</p><h1>${config.title}</h1><p class="intro">${config.intro}</p></div><div class="library-tools"><label class="search-label" for="study-search">Search questions, answers, or question number</label><input id="study-search" type="search" placeholder="Try: torque, PLC, or 19" autocomplete="off"><div class="library-filters"><label for="study-topic">Topic<select id="study-topic"><option value="">All topics</option>${topics.map(t=>`<option>${esc(t)}</option>`).join('')}</select></label><label class="review-filter"><input type="checkbox" id="only-review"> Review later <span id="review-total">(${review.size})</span></label></div><div class="library-controls"><span id="study-count" role="status"></span>${view!=='mnemonics'?'<button class="text-button" id="toggle-answers">'+(view==='answers'?'Hide answers':'Reveal answers')+'</button>':''}</div></div>${view==='guide'?'<p class="library-notice">62 answered items + 3 figure references. Questions 13, 14, and 60 are retained with missing-figure labels.</p>':'<p class="library-notice">62 entries from your Answers & Mnemonics PDF. Original study-guide numbers are preserved; 13, 14, and 60 are omitted in that source.</p>'}<div id="study-list"></div><div class="source-downloads"><p>Source documents</p><a href="documents/${config.pdf}" target="_blank" rel="noopener">Open original ${view==='guide'?'study guide':'answers & mnemonics'} PDF ↗</a><a href="documents/remaining-practice.pdf" target="_blank" rel="noopener">Open remaining-questions practice PDF ↗</a></div>`;
  const list=app.querySelector('#study-list');
  function filtered(){return items.filter(q=>(!topic||q.topic===topic)&&(!onlyReview||review.has(q.number))&&(!search||(String(q.number)===search)||`${q.topic} ${q.question} ${q.answer} ${q.remember} ${q.mnemonic} ${q.reference}`.toLowerCase().includes(search)));}
  function paint() {
  ...
  app.querySelector('#study-search').oninput=e=>{search=e.target.value.trim().toLowerCase();paint();};
